...
#define TMR_LOAD			100000000 // reset value  for timer 0
#define TMR_LOAD1			10000000 // reset value for timer 1

#define TMR_LOAD10X			10000000
#define TMR_LOAD20X			5000000
#define TMR_LOAD30X			3333333
#define TMR_LOAD60X			1666666
...
void SW_Intr_Handler(void *InstancePtr){
	...
	else if (sw_value == 1){ //0001
		XTmrCtr_SetResetValue(&TMRInst, 0, TMR_LOAD10X);
	}
	else if (sw_value >= 2 && sw_value < 4){ //001-
		XTmrCtr_SetResetValue(&TMRInst, 0, TMR_LOAD20X);
	}
	else if (sw_value >= 4 && sw_value < 8){ //01--
		XTmrCtr_SetResetValue(&TMRInst, 0, TMR_LOAD30X);
	}
	else{ //1---
		XTmrCtr_SetResetValue(&TMRInst, 0, TMR_LOAD60X);
	}
...
int main (void)
{
  ...
  XGpio_DiscreteWrite(&LEDInst, 1, 8); // initially in regular mode
...
int IntcInitFunction(u16 DeviceId, XTmrCtr *TmrInstancePtr, XTmrCtr *TmrInstancePtr1, XGpio *GpioInstancePtr, XGpio *GpioInstancePtr1)
{
	...
	XScuGic_Enable(&INTCInst, INTC_GPIO_INTERRUPT_ID); // Buttons
	XScuGic_Enable(&INTCInst, INTC_GPIO_INTERRUPT_ID_2); // Switches
